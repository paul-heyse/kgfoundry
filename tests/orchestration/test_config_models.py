...
def test_index_cli_immutability() -> None:
...
def test_index_cli_equality() -> None:
...
def test_index_cli_inequality() -> None:
...
def test_custom_construction() -> None:
    ...
    assertions.expect_true(config.fail_on_warnings, reason="config.fail_on_warnings should be True")
...
def test_artifact_config_inequality() -> None:
    """Test inequality comparison."""
    config1 = ArtifactValidationConfig(strict_mode=True, fail_on_warnings=False)
...
def test_artifact_validation_defaults() -> None:
...
def test_both_flags_false() -> None:
