...
def test_empty_chunks_view_creates_relation() -> None:
    ...
    try:
        ...
        row = conn.execute(sql_relation_exists(), [VIEW_CHUNKS, VIEW_CHUNKS]).fetchone()
        if row is None:
...
def test_chunks_materialized_sql_executes(tmp_path: Path) -> None:
    ...
    conn = duckdb.connect(database=":memory:")
    try:
        parquet_literal = str(parquet).replace("'", "''")
        literal = f"'{parquet_literal}'"
...
def test_relation_exists_with_missing_relation() -> None:
    ...
    conn = duckdb.connect(database=":memory:")
    try:
        row = conn.execute(sql_relation_exists(), ["missing", "missing"]).fetchone()
