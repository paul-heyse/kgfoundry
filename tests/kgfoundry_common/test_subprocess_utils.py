...
def test_simple_command_success() -> None:
...
def test_command_with_output() -> None:
...
def test_command_failure_raises_error() -> None:
...
def test_command_with_stderr() -> None:
    ...
    assertions.expect_true(exc_info.value.stderr is not None, reason="stderr should be captured")
...
def test_timeout_error_has_details() -> None:
...
def test_cwd_resolved_to_absolute_path(tmp_path: Path) -> None:
...
def test_env_none_inherits_parent() -> None:
...
def test_env_dict_mapping() -> None:
...
def test_subprocess_error_message_includes_command() -> None:
    """Test that error message includes command."""
    with pytest.raises(SubprocessError) as exc_info:
        run_subprocess(["false"], timeout=5)
    assertions.expect_true("false" in str(exc_info.value), reason="error should mention command")


def test_timeout_error_message_includes_command() -> None:
    """Test that timeout error includes command."""
    with pytest.raises(SubprocessTimeoutError) as exc_info:
        run_subprocess(["sleep", "10"], timeout=1)
    assertions.expect_true("sleep" in str(exc_info.value), reason="error should mention command")


def test_python_script_with_args() -> None:
...
def test_shell_command_via_python() -> None:
...
def test_echo_various_inputs(value: str) -> None:
