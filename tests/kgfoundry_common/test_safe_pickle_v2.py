...
def test_dump_and_load_allowed_types(wrapper: SignedPickleWrapper) -> None:
    """Test that allowed types round-trip correctly."""
    for data in _ALLOWED_VALUES:
        buffer = io.BytesIO()
...
def test_signature_verification_success(wrapper: SignedPickleWrapper) -> None:
...
def test_different_key_fails_verification(signing_key: bytes) -> None:
...
def test_disallowed_type_rejected_on_dump(wrapper: SignedPickleWrapper) -> None:
...
def test_disallowed_type_rejected_on_load() -> None:
...
def test_short_key_warning_logged(caplog: LogCaptureFixture) -> None:
...
def test_nested_container_depth_limit(wrapper: SignedPickleWrapper) -> None:
...
def test_complex_nested_structure(wrapper: SignedPickleWrapper) -> None:
    ...
    assertions.expect_equal(loaded, cast("dict[str, object]", data))
    ...
    assertions.expect_false(bool(config["debug"]))
