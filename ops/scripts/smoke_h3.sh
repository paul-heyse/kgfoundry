...
if ! curl --http3-only -V >/dev/null 2>&1; then
  echo "curl binary is missing HTTP/3 support (needs nghttp3/quiche build)" >&2
  exit 2
fi

echo "==> curl --http3 -I ${TARGET}${READY_PATH}" >&2
curl "${CURL_FLAGS[@]}" --http3 -I "${TARGET}${READY_PATH}"

echo "==> curl --http3-only -I ${TARGET}${READY_PATH}" >&2
curl "${CURL_FLAGS[@]}" --http3-only -I "${TARGET}${READY_PATH}"

echo "==> curl --http3 -N ${TARGET}${STREAM_PATH} (timeout ${STREAM_TIMEOUT}s)" >&2
curl "${CURL_FLAGS[@]}" --http3 -N --max-time "${STREAM_TIMEOUT}" "${TARGET}${STREAM_PATH}"
