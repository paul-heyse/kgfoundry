...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--allowlist",
        default=str(ALLOW_FILE),
        help="Path to graph allowlist (cycles/edges)",
    )
    p.add_argument(
        "--fail-on-cycles",
        action="store_true",
...
def _cycle_generator(graph: DiGraph, length_bound: int) -> Iterator[list[str]]:
    ...
    if length_bound > 0:
        try:
            return cast(
                "Iterator[list[str]]",
                nx.simple_cycles(graph, length_bound=length_bound),
            )
        except TypeError:
            pass
...
def _log_run_summary(
    *,
    ...
    if use_cache:
        LOGGER.info(
            "cache summary: per-package built=%d cached=%d; cache_dir=%s",
            built,
            cached,
            cache_dir,
        )
