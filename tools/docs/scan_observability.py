...
def _apply_runbooks(logs: list[dict[str, object]], runbooks: dict[str, str]) -> None:
    """Attach runbook URLs to log rows when taxonomy entries match."""
    if not runbooks:
        return

    for row in logs:
        current_runbook = cast("str | None", row.get("runbook"))
        if current_runbook:
            continue
        template = cast("str", row.get("message_template") or "")
...
def _write_outputs(
    ...
    logs: list[LogRow],
    log_payload: list[dict[str, object]],
...
def main() -> int:
    ...
    log_payload = [cast("dict[str, object]", asdict(entry)) for entry in logs]
    _apply_runbooks(log_payload, runbooks)
    _write_outputs(metrics, logs, log_payload, traces, lints)
