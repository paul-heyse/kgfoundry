...
| `EDGE_MODE` value      | Flow                                                                    | When to use |
|------------------------|-------------------------------------------------------------------------|-------------|
| `nginx-terminate` (default) | Client ⇢ **nginx (HTTP/3/TLS)** ⇢ Hypercorn (**HTTP/1.1 on 127.0.0.1:8080**) | Stable, friendly operations surface. nginx handles Alt-Svc, buffering, ACME, observability. |
| `h3-pass-through`      | Client ⇢ **nginx stream pass-through (UDP+TCP 443)** ⇢ Hypercorn (**HTTP/3 on origin**) | Literal QUIC to Python. Requires certificates on Hypercorn and UDP 443 open end-to-end. |
...
* `ops/hypercorn.toml` — shared Hypercorn tuning. The pass-through bind/quic
  ...
  `EDGE_MODE=h3-pass-through`.
* nginx configs:
  * `ops/nginx/mcp.h3-terminate.conf` (HTTP/3 terminates at nginx).
  * `ops/nginx/mcp.h3-pass-through.stream.conf` (TCP/UDP stream proxy).
* systemd: `ops/systemd/hypercorn.service` exports `EDGE_MODE`, and the optional
...
make dev-up EDGE_MODE=nginx-terminate
make dev-up EDGE_MODE=h3-pass-through \
...
targets and the systemd unit. It inspects `EDGE_MODE` and adds QUIC binds +
...
1. Install the assets:
   ...
   sudo install -m0644 ops/nginx/mcp.h3-terminate.conf /etc/nginx/conf.d/mcp.conf
   sudo install -m0644 ops/nginx/mcp.h3-pass-through.stream.conf /etc/nginx/stream.d/mcp.stream.conf
   ```
2. Enable HTTP/3 at the edge (open UDP 443, ensure nginx built with QUIC).
3. Set `EDGE_MODE` in `/etc/systemd/system/hypercorn.service` (drop-in) before
   `systemctl daemon-reload && systemctl restart hypercorn`.
4. Reload nginx via `make reload-nginx` (runs syntax check first).

## 4. Smoke tests

`ops/scripts/smoke_h3.sh` hits `/mcp/readyz` and `/sse` with `curl --http3`.
`make smoke-h3 SMOKE_HOST=https://mcp.example.com` wraps the script. Expect:
...
5. **Hypercorn refuses to start in pass-through mode** — verify cert/key paths
   are readable by the `app` user and that `EDGE_MODE=h3-pass-through` is exported
...
Keep `OPERATIONS.md`, `ops/hypercorn.toml`, and `ops/nginx/*` in sync when making
future topology tweaks. The `smoke_h3.sh` transcript in `SMOKE.txt` should be
regenerated whenever you change edge behavior.
